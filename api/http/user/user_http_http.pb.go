...
const OperationUserStudentSign = "/http.user.User/StudentSign"
...
type UserHTTPServer interface {
	...
	// StudentSign学生签到
	StudentSign(context.Context, *StudentSignRequest) (*StudentSignReply, error)
}
...
func RegisterUserHTTPServer(s *http.Server, srv UserHTTPServer) {
	...
	r.POST("/v1/user/studentsign", _User_StudentSign0_HTTP_Handler(srv))
}
...
func _User_StudentSign0_HTTP_Handler(srv UserHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in StudentSignRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationUserStudentSign)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.StudentSign(ctx, req.(*StudentSignRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*StudentSignReply)
		return ctx.Result(200, reply)
	}
}
...
type UserHTTPClient interface {
	...
	StudentSign(ctx context.Context, req *StudentSignRequest, opts ...http.CallOption) (rsp *StudentSignReply, err error)
}
...
func (c *UserHTTPClientImpl) StudentSign(ctx context.Context, in *StudentSignRequest, opts ...http.CallOption) (*StudentSignReply, error) {
	var out StudentSignReply
	pattern := "/v1/user/studentsign"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationUserStudentSign))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
